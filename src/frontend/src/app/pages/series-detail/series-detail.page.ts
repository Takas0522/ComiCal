    ...
    <app-page-layout>
      <div data-testid="page-series-detail" class="py-5">
        @if (isLoading()) {
          ...
          @if (!series()!.volumes?.length) {
            ...
            <ul class="flex flex-col gap-2">
              @for (vol of series()!.volumes; track vol.volumeId) {
                <li
                  class="px-4 py-3 flex items-center gap-3 rounded-xl"
                  style="background: var(--color-surface); box-shadow: var(--shadow-card)"
                >
                  @if (vol.thumbnailUrl) {
                    <img
                      [src]="vol.thumbnailUrl"
                      [alt]="series()!.title + ' 第' + (vol.volumeNumber ?? '?') + '巻'"
          ...
          <p class="py-16 text-center" style="color: var(--color-text-secondary)">
            シリーズが見つかりませんでした。
          </p>
        }
      </div>
